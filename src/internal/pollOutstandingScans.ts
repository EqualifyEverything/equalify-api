...
export const pollOutstandingScans = async ({ request, reply }) => {
  ...
  try {
    ...
    if(!jobs){ console.log(`No open scans found, exiting...`); return; }
    ...
    for (const job of sortedJobIds) {
      ...
      const scanResults = await fetch(
        `https://scan${isStaging ? "-dev" : ""}.equalify.app/results/${
          job.job_id
        }`,
      ...
      if (["delayed", "active", "waiting"].includes(status)) {
        ...
        if (result == null || result == "null" || result == undefined) {
          // if result is complete but the data is empty, set processing to false;
          await db.query({
        ...
        const nodeIds = await ingestScanData(
          db,
          result,
          job.job_id,
          job.url_id,
