...
const scanProcessor = async ({ result, scan }) => {
    ...
    for (const row of result.nodes) {
        row.id =
            (await pgClient.query({
                text: `SELECT "id" FROM "nodes" WHERE "user_id"=$1 AND "html"=$2 AND "targets"=$3 AND "url_id"=$4`,
                values: [jwtClaims.sub, row.html, row.targets, row.url_id],
            })).rows?.[0]?.id
            ??
            (await pgClient.query({
                text: `INSERT INTO "nodes" ("user_id", "html", "targets", "url_id") VALUES ($1, $2, $3, $4) RETURNING "id"`,
                values: [jwtClaims.sub, row.html, row.targets, row.url_id],
            })).rows?.[0]?.id;
    }
